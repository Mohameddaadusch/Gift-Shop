...
const CategoryShowcase: React.FC = () => {
  ...
  const handleCategoryClick = (category: string) => {
    navigate(`/all-gifts?category=${encodeURIComponent(category)}`);
  };


  useEffect(() => {
    fetch('/Data/Category_Images_and_Info.csv')
      ...
      .then((csvText) => {
        ...
        setCategories(parsed.data as Category[]);
      })
      .catch((error) => {
        console.error('Error loading category CSV:', error);
      });
  }, []);
